...
async def create_new_inventory_item(item: InventoryItem, session: Annotated[Session, Depends(get_session)], producer: Annotated[AIOKafkaConsumer, Depends(get_kafka_producer)] ):
    ...
    # print(f"Added into the database:{new_item}")
    return item
...
async def single_inventory_item(item_id: int, session: Annotated[Session, Depends(get_session)]):
...
#     try:
#         return update_single_inventory_item(item_id, to_update_item=item, session=session)
#     except HTTPException as e:
#         raise e
#     except Exception as e:
#         raise HTTPException(status_code=500, detail=str(e))